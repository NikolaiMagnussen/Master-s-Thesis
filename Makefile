...
br0: 
	@ip link | grep ": br0: .*state UP" || ($(info "Bridge not available ($@) - creating one") sudo ip link add br0 type bridge && sudo ip addr add 10.0.0.1/24 dev br0 && sudo ip link set dev br0 up)

tap%: br0
	@ip link | grep ": $@: .*state UP" || ($(info "Tap not available ($@) - creating one") sudo ip tuntap add $@ mode tap && sudo ip link set $@ master $< && sudo ip link set dev $@ up)
